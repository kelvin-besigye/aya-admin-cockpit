...
/**
 * 👑 AYA BUS COMMAND BOARD: Security & Crypto (Level 4: Sovereign Edition)
 * ------------------------------------------------------------------
 * Module: Control Centre
 * File: SectorSecurity.jsx
 * * DESCRIPTION:
 * The physical command board for Sector 8. Governs zero-trust security policies,
 * including authentication gating, session lifespans, and intrusion lockouts.
 * * UPGRADES:
 * - Bifurcated Hardware Injection: Seamlessly renders Steppers for timeouts and KillSwitches for toggles.
 * - Hardware Iconography: Natively maps Key (2FA), UserX (Lockout), and Clock (Session) icons based on config keys.
 * - Schema-Driven Render Engine: Infinitely scalable without frontend updates.
 * - Zero-Trust Warning Banners: Contextually warns Admins of the blast radius of downgrading security.
 */

const SectorSecurity = ({ 
    sectorState = {}, 
    onSettingChange 
}) => {
    // 1. Isolate the schema for the Security sector
    const sectorSchema = MASTER_SCHEMA[CONTROL_SECTORS.SECURITY.id] || {};

    // 2. Icon Mapper for visual cognitive routing
    const getSettingIcon = (settingId) => {
        const id = settingId.toLowerCase();
        if (id.includes('2fa') || id.includes('auth') || id.includes('otp')) return Key;
        if (id.includes('lockout') || id.includes('fail')) return UserX;
        if (id.includes('session') || id.includes('time')) return Clock;
        return Lock;
    };
    ...
    return (
        <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '24px' }}>
                <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: 'color-mix(in srgb, var(--status-success) 10%, transparent)', color: 'var(--status-success)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                ...
                <div>
                    ...
                    <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0, fontWeight: '600', lineHeight: '1.5', maxWidth: '650px' }}>
                        Establish the Zero-Trust boundaries for the AyaBus OS. Manage authentication friction, brute-force mitigation, and administrative session lifecycles.
                    </p>
                </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', borderRadius: '12px', background: 'color-mix(in srgb, var(--status-danger) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--status-danger) 20%, transparent)' }}>
                <ShieldAlert size={20} color="var(--status-danger)" style={{ flexShrink: 0 }} />
                <span style={{ fontSize: '13px', color: 'var(--status-danger)', fontWeight: '700', lineHeight: '1.4' }}>
                    <strong>ZERO-TRUST NOTICE:</strong> Disabling mandatory 2FA or increasing the maximum failed login threshold severely degrades the platform's defense against credential stuffing and unauthorized access.
                </span>
            </div>
