...
import { 
    ...
    Percent, CalendarDays, Gavel, Users
} from 'lucide-react';
...
/**
 * 👑 AYA BUS COMMAND BOARD: Partner Governance (Level 4: Sovereign Edition)
 * ------------------------------------------------------------------
 * Module: Control Centre
 * File: SectorPartners.jsx
 * * DESCRIPTION:
 * The physical command board for Sector 5. Controls the B2B laws of the ecosystem: 
 * global revenue share defaults, automated settlement windows, and SLA penalty strikes.
 * * UPGRADES:
 * - Schema-Driven Architecture: Zero hardcoded forms; natively loops through the NoSQL blueprint.
 * - Dynamic Icon Ontology: Intelligently assigns Calendar, Percent, and Gavel icons based on the configuration ID.
 * - Blast Radius Badging: Visually isolates HIGH risk parameters that affect fleet operator relations.
 * - Mathematical Stepper Lock: Relies entirely on the Sovereign Stepper to enforce strict min/max payout physics.
 */
...
const SectorPartners = ({ 
    ...
    // 1. Isolate the schema for the Partners sector
    const sectorSchema = MASTER_SCHEMA[CONTROL_SECTORS.PARTNERS.id] || {};

    // 2. Icon Mapper for visual cognitive routing
    const getSettingIcon = (settingId) => {
        const id = settingId.toLowerCase();
        if (id.includes('share') || id.includes('percent') || id.includes('rev')) return Percent;
        if (id.includes('window') || id.includes('day') || id.includes('settlement')) return CalendarDays;
        if (id.includes('strike') || id.includes('sla') || id.includes('penalty')) return Gavel;
        return Users;
    };
        ...
        <div style={{ 
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '16px', borderBottom: '1px solid var(--border-subtle)', paddingBottom: '24px' }}>
                ...
                <div>
                    <h1 style={{ fontSize: '20px', fontWeight: '900', color: 'var(--text-main)', margin: '0 0 6px 0', letterSpacing: '-0.5px' }}>
                        Partner Governance & B2B Rules
                    </h1>
                    <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0, fontWeight: '600', lineHeight: '1.5', maxWidth: '650px' }}>
                        Establish the default operating agreements for all bus operators on the network. Modifications to revenue shares and settlement windows require strict operational alignment.
                    </p>
                </div>
            </div>

            {/* --- GLOBAL WARNING BANNER (B2B Impact) --- */}
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '16px', borderRadius: '12px', background: 'color-mix(in srgb, var(--status-warning) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--status-warning) 25%, transparent)' }}>
                ...
                <span style={{ fontSize: '13px', color: 'var(--status-warning)', fontWeight: '700', lineHeight: '1.4' }}>
                    <strong>PARTNER IMPACT:</strong> Changing the default Revenue Split here only affects newly onboarded partners or global resets. Individual partner overrides in the Partner Portal will remain active.
                </span>
            </div>
                ...
                {Object.values(sectorSchema).map((settingDef) => {
                    ...
                    return (
                        ...
                        >
                            ...
                            {/* Hardware Injection (Strictly Steppers for Partner Economics) */}
